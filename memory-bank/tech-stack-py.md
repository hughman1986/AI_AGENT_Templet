...
* 常見搭配（Common Integrations）：
  * ORM：SQLAlchemy 2.x
  * 輕量資料存取：SQLModel / 原生 SQLAlchemy Core（依專案需求選擇）
  * PostgreSQL Driver：psycopg 3
  * 驗證框架：Pydantic v2
  * 日誌框架：structlog / logging（依專案需求選擇）
  * 依賴注入：FastAPI Depends / 手動組裝

### 桌面應用程式（Desktop Application）

* UI 框架：PySide6 / Tkinter（依專案選擇）
* 架構模式：MVC / MVVM（依 UI 框架與專案規模選擇）
* UI 元件庫：依專案需求評估
